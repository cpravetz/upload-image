...
Template.editYourAvatarModal.rendered = function () {
	widthAvatar = Session.get('UI_Width');
	heightAvatar = Session.get('UI_Height');

	if(widthAvatar >= heightAvatar) {
		ratioImage = Math.ceil(widthAvatar/heightAvatar).toString() + ':1';
	} else {
		ratioImage = '1:' + Math.ceil(heightAvatar/widthAvatar).toString();
	}
...
Template.editYourAvatarModalBody.rendered = function () {
	...
	heightAvatar = Session.get('UI_Height');
	if(widthAvatar >= heightAvatar) {
		ratioImage = '1:' + Math.ceil(widthAvatar/heightAvatar).toString();
	} else {
		ratioImage = Math.ceil(heightAvatar/widthAvatar).toString() + ':1';
	}
...
var processChangeAvatar = function (tmp) {
	...
	Meteor.call(sMethods, canvas.toDataURL(), function (err, res) {
		if (err) {
			error.html(createAlertDanger(err.message));
			Meteor.setTimeout(function () {
				error.html('');
